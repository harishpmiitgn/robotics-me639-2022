...
k_p = np.array([0.001, 0.001, 0.01])
k_i = np.array([0.0001, 0.0001, 0.0001])
k_d = np.array([0.001, 0.0001, 0.0001])

error				= np.zeros_like(StanfordRobot.q)
error_previous		= np.zeros_like(StanfordRobot.q)
integral_error		= np.zeros_like(StanfordRobot.q)
derivative_error	= np.zeros_like(StanfordRobot.q_dot)
...
def getEffort(error : np.ndarray, integral_error:np.ndarray, derivative_error : np.ndarray) -> np.ndarray :
	...
	np.copyto(
		effort,
		np.sign(effort) * 0.0001,
		where= np.abs(effort) > 0.0001
	)
...
def getStateDerivative(t:float, state:np.ndarray, tau:np.ndarray) -> np.ndarray :
	...
	return np.append(
		q_dot,
		StanfordRobot.getQDDot(q, q_dot, tau)
	)
...
def proceedTimeStep(t, q:np.ndarray, q_dot:np.ndarray, q_d:np.ndarray, q_d_dot:np.ndarray) -> float:
	...
	StanfordRobot.q		= solution.y[:3, -1]
	StanfordRobot.q_dot	= solution.y[3:, -1]
