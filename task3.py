...
def rungekutta4(f,y0,t,tau,l1,l2,m1,m2):
        ...
        for i in range(n-1):
            ...
            k1 = f(y[i], t[i], taui,l1,l2,q1f,q2f,m1,m2)
            k2 = f(y[i] + k1 * h / 2., t[i] + h / 2., taui,l1,l2,q1f,q2f,m1,m2)
            k3 = f(y[i] + k2 * h / 2., t[i] + h / 2., taui,l1,l2,q1f,q2f,m1,m2)
            k4 = f(y[i] + k3 * h, t[i] + h,taui,l1,l2,q1f,q2f,m1,m2)
            y[i+1] = y[i] + (h / 6.) * (k1 + 2*k2 + 2*k3 + k4)
            #y[i+1] = y[i]+(t[i+1] - t[i])*f(y[i],t[i],tau,l1,l2,m1,m2)
        #y = np.round(y,3)
        return y    
def f(y,t,tau,l1,l2,q1f,q2f,m1,m2): 
            g = 9.81
            q1=y[0]
            q2=y[1]
            q1dot=y[2]
            ...
            #print(C)
            #-np.dot(C,qdot)
            x1a,y1a,x2a,y2a = fk_2R(q1,q2,l1,l2)
            x1d,y1d,x2d,y2d= fk_2R(q1f,q2f,l1,l2)
            e = x2d - x2a + y2d - y2a#np.sqrt((x2d-x2a)**2+(y2d-y2a)**2)
            e1 = x1d - x1a + y1d - y1a#np.sqrt((x1d-x1a)**2+(y1d-y1a)**2)
            K = np.array([[100 ,0],[0 ,100]])
            clx=-np.dot(D,np.dot(K,np.array([[e1],[e]])))
            clj=np.dot(D,np.dot(K,np.array([[q1f-q1],[q2f-q2]])))
            tau = Cd+phi
            qddot = np.dot(Dinv,(-Cd-phi+tau+clj))
...
#Circular Trajectory
x = 0.7+0.3*np.cos(10*t)   # Different Speeds can be tried by changing coefficient of t

y = 0.7+0.3*np.sin(10*t)    # Different Speeds can be tried by changing coefficient of t
...
y = 0.05*np.sin(20*t)+0.9    # Different Speeds can be tried by changing coefficient of t
x = 0.3*y

y = 1.2*np.ones(len(t))  # one desired position
x =  1.2*np.ones(len(t))
xd = 1.2
yd = 1.2

# ws,q1f,q2f = ik_2R(0.9,0.3*0.9,elbow,l1,l2)  # mean position
ws,q1f,q2f = ik_2R(xd,yd,elbow,l1,l2) 
###
#n = np.size(x)
...
dq1 = np.pi/2           # change initial position here
dq2 = np.pi/2           # change initial position here
y0 = np.round(np.array([q[0][0]+dq1,q[0][1]+dq2,qdot[0][0],qdot[0][1]]),3)
